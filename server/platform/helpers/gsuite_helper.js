...
module.exports = class GoogleSuiteHelper {
    ...
    async getUserProfile() {
        try {
            const response = await people.people.get({
                resourceName: 'people/me', personFields: 'names,emailAddresses', auth: this.oauth2Client,
            });
            return response.data;
        } catch (error) {
            return await this.logoutURL(error.response);
        }
    }
    ...
    async logout(params) {
        try {
            const { access_token } = params;
            const response = await this.revokeToken(access_token);
            return await this.logoutURL(response);
        } catch (error) {
            return await this.logoutURL(error.response);
        }
    }

    async logoutURL(params) {
        let logoutUrl = '';
        if (params.status === 200 || params.status === 400 || params.status === 401) {
            logoutUrl = `${process.env.APP_BASE_URL}`;
        }
        if (params.status === 403) {
            throw { status_code: params.status, message: params.data.error.message };
        }
        return { logoutUrl, isVerified: false };
    }
};
